(function () {
    ...
    const saveCart = (cart) => {
        activeStore.setItem(storageKey, JSON.stringify(normalizeCart(cart)));
    };
    ...
    const removeItem = (productId) => {
        const filtered = readCart().filter((entry) => entry.id !== productId);
        saveCart(filtered);
    };
            ...
            <tr data-product-id="${item.id}">
                ...
                <td>
                    <div class="input-group input-group-sm cart-qty-group">
                        ...
                        <input type="number" class="form-control text-center js-cart-qty" min="1" value="${quantity}">
                        <button class="btn btn-outline-light js-cart-inc" type="button">+</button>
                    </div>
                </td>
                <td>${formatPrice(lineTotal)}</td>
                <td>
                    <button type="button" class="btn btn-sm btn-danger js-cart-remove">Supprimer</button>
                </td>
            </tr>
        `;
    };
    ...
    const initCartActions = () => {
        ...
        itemsElement.addEventListener("change", (event) => {
            const target = event.target;
            if (!(target instanceof HTMLInputElement) || !target.classList.contains("js-cart-qty")) {
                return;
            }
            ...
            const productId = parseInt(row.dataset.productId || "0", 10);
            const quantity = parseInt(target.value || "1", 10);
